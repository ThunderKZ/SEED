document.addEventListener('DOMContentLoaded', function() {
    ...
    (function() {
        ...
        if (isMobile) {
            ...
            fitScale = Math.min(scaleW, scaleH, 1);
        } else {
            // PC端：只按宽度适配，高度自由（可根据需求调整）
            fitScale = 1;
            // 【可选】PC端完全不缩放：fitScale = 1;
        }
        ...
        if (zoomOut) {
            zoomOut.addEventListener('click', () => {
                let newScale = Math.max(INIT_FIT_SCALE, scale - 0.2);
                setScaleWithCenter(newScale, window.innerWidth/2, window.innerHeight/2);
            });
        }
